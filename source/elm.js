...
Elm.Main.make = function (_elm) {
   ...
   var UpdatePositionTitle = function (a) {    return {ctor: "UpdatePositionTitle",_0: a};};
   var UpdateCompanyName = function (a) {    return {ctor: "UpdateCompanyName",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.type$("text")
                      ,$Html$Attributes.placeholder("Company Name")
                      ,$Html$Attributes.value(model.companyName)
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (text) {    return A2($Signal.message,address,UpdateCompanyName(text));})]),
              _U.list([]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.type$("text")
                      ,$Html$Attributes.placeholder("Position Title")
                      ,$Html$Attributes.value(model.positionTitle)
                      ,A3($Html$Events.on,
                      "input",
                      $Html$Events.targetValue,
                      function (text) {
                         return A2($Signal.message,address,UpdatePositionTitle(text));
                      })]),
              _U.list([]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Reset)]),_U.list([$Html.text("Reset")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Company Name: ",model.companyName))]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Position Title: ",model.positionTitle))]))]));
   });
   var initialModel = {companyName: "",positionTitle: ""};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "NoOp": return model;
         case "UpdateCompanyName": return _U.update(model,{companyName: _p0._0});
         case "UpdatePositionTitle": return _U.update(model,{positionTitle: _p0._0});
         default: return initialModel;}
   });
   var Model = F2(function (a,b) {    return {companyName: a,positionTitle: b};});
   var inbox = $Signal.mailbox(NoOp);
   var modelSignal = A3($Signal.foldp,update,initialModel,inbox.signal);
